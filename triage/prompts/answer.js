...
function getAnswerSystemPrompt(mode = 'homeowner') {
...
function getAnswerUserPrompt(message, routerOutput, context) {
  const contextParts = [];
...
function getPostureGuidance(postures) {
  const guidance = [];

  if (postures.includes('triager')) {
    guidance.push(
      'TRIAGE MODE: Prioritize actions by urgency. Help user understand what needs immediate attention vs. what can wait.'
    );
  }

  if (postures.includes('risk_manager')) {
    guidance.push(
      'RISK MANAGEMENT MODE: This is a safety-sensitive topic. Be conservative. Explicitly recommend professional inspection if there is any risk to life, property, or code compliance.'
    );
  }

  if (postures.includes('explainer')) {
    guidance.push(
      'EXPLAINER MODE: Help the user understand what is happening and why. Use analogies if helpful.'
    );
  }

  if (postures.includes('optimizer')) {
    guidance.push(
      'OPTIMIZER MODE: Help the user understand trade-offs between cost, quality, and time. Provide options at different price points.'
    );
  }
