-- ========================================
-- HomeProHub Database Schema (Fixed)
-- ========================================
-- This version drops existing views before recreating them

-- ========================================
-- DROP EXISTING VIEWS FIRST
-- ========================================
DROP VIEW IF EXISTS job_board_view CASCADE;
DROP VIEW IF EXISTS homeowner_rating_summary CASCADE;
...
-- Homeowner Rating Summary View
CREATE VIEW homeowner_rating_summary AS
SELECT
  homeowner_contact,
  homeowner_id,
...
-- Job Board View
CREATE VIEW job_board_view AS
SELECT
  j.*,
  COALESCE(up.full_name, up.first_name || ' ' || up.last_name) as homeowner_name,
...
GROUP BY j.id, up.full_name, up.first_name, up.last_name, up.phone;
...
-- Increment bid count
CREATE OR REPLACE FUNCTION increment_bid_count(job_id UUID)
RETURNS void AS $$
BEGIN
  UPDATE job_postings
...
-- Next step: Run enable-rls.sql to secure the database
